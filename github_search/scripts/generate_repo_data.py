...
class RepoDataGenerator:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "license": (
                        data.get("license", {}).get("name", "Unknown")
                        if data.get("license")
                        else "Unknown"
                    ),
    ...
    async def generate_repo_data(self) -> List[Dict]:
        ...
        for i in range(0, len(repo_list), batch_size):
            ...
            print(
                f"   Repositories {i+1}-{min(i+batch_size, len(repo_list))} of {len(repo_list)}"
            )
            ...
            for repo, result in zip(batch, batch_results):
                ...
                if processed % 50 == 0:
                    print(
                        f"   📊 Progress: {processed}/{len(repo_list)} ({processed/len(repo_list)*100:.1f}%)"
                    )
    ...
    def save_repo_data(self, repo_data: List[Dict]):
        ...
        with open(summary_filename, "w") as f:
            f.write(
                f"Thrust Repository Summary - Generated {datetime.now().isoformat()}\n"
            )
            f.write("=" * 70 + "\n\n")
